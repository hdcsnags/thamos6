...
async function fetchRansomwareLiveData(): Promise<any> {
  const cacheKey = "ransomware_live:all_victims";
  const cached = await getCachedData(cacheKey);
  if (cached) return cached;

  try {
    const response = await fetch("https://api.ransomware.live/victims", {
...
async function syncVictimData(victims: RansomwareVictim[]): Promise<void> {
  for (const victim of victims) {
    const { data: existingVictim } = await serviceClient
      .from("ransomware_victims")
      .select("id, victim_name")
      .eq("victim_name", victim.victim_name)
      .eq("discovery_date", victim.discovery_date)
      .maybeSingle();
    ...
    const { data: insertedVictim, error: victimError } = await serviceClient
      ...
      .insert({
        ...
        discovery_date: victim.discovery_date,
        country_code: victim.country_code,
        country_name: victim.country_name,
        sector: victim.sector,
        ransom_amount: victim.ransom_amount,
        leak_site_url: victim.leak_site_url,
        screenshot_url: victim.screenshot_url,
        data_leaked: victim.data_leaked ?? false,
      ...
      if (!groupId) {
        const { data: newGroup } = await serviceClient
          ...
          .insert({
            ...
            is_active: true,
            severity_level: "high",
            first_seen: victim.discovery_date,
            last_activity: victim.discovery_date,
          })
          .select("id")
          .single();
      ...
      if (groupId) {
        await serviceClient
          ...
          .insert({
            ...
            attributed_date: victim.discovery_date,
          });
      }
    }
  }
