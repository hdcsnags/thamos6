...
export function DesktopLayout() {
  ...
  useEffect(() => {
    if (desktop.bootComplete && Object.keys(desktop.windows).length === 0) {
      // Try restoring previous session layout
      const restored = desktop.restoreSavedLayout();
      if (!restored) {
        // First boot — open default windows
        desktop.openWindow({
          appId: 'terminal',
          title: 'Terminal',
          position: { x: 100, y: 60 },
  ...
  // Track recently closed windows for Ctrl+Shift+T
  ...
  const reopenLastClosed = useCallback(() => {
    ...
    desktop.openWindow({ appId: last.appId as any, title: last.title });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Skip remaining shortcuts if user is typing in an input/terminal
