...
// Accent color logic (identity colors live in icon tiles only):
//   core surfaces  → distinct hues: terminal=green, vps=amber, scanner=accent, browser=blue
//   intel          → workshop=pink, intel=orange
//   workspace tools→ teal (cases, files, editor)
//   transform utils→ cyan (decoder, defang, ioc-extractor, bulk-lookup)
//   content analyzers → amber (email-analyzer, extension-scanner, doc-analyzer)
//   system         → slate (monitor, settings)
//   result windows → accent (uniform: all are scanner output)
