...
> **Last Updated:** 2026-08-20 by Claude Fable 5 (full-stack assessment → PII gate, email-first Email Analyzer workbench, calibrated persistence, desktop token/honesty pass, container-query reflow)
> 
...
> - **Status:** Sender geo/ASN card, dual-source threat-intel widget, and Export actions (JSON/print) **shipped 2026-08-20d** — see Sprint Log. Reverse-DNS/PTR + 4-list blacklist rollup and rendered "Decoded Email Payload" thread view remain backlog, not started. **The anti-pattern's privacy gate shipped 2026-08-20e**: recipient-bound URLs are withheld from external URL scanners entirely (domain-only enrichment), urlscan submissions default to `unlisted`, and emailrep lookups are sender-side-only — see Sprint 2026-08-20e.
...
**Scope:** User asked for a full assessment of the Desktop theme, Email Scanner, and IP scanner, then granted a blanket mandate ("attack it as you see fit") plus two specific asks: fix the PII shipping, and rework the Email Analyzer's "clonky" triple-pane UX into an email-first view with the useful facts (sender IP + T6 score, links/QR/doc URLs) visible up front.

**Shipped (6 commits, pushed to main):**
1. **PII boundary closed** (`9de9e77`, `analyze-email` + `threat-intel` + `email-verdict` deployed). The assessment confirmed three live leaks: ENRICH ALL submitted recovered URLs (which can embed the recipient's address) to urlscan.io with `visibility:"public"` AND to VirusTotal community, and sent recipient addresses verbatim to emailrep.io. Now: URLs with `recipientBinding.detected` (exact recipient OR tenant-domain) never reach `/threat-intel/url` — their host gets `/domain` enrichment instead, with an explicit `piiWithheld` marker in the response (`withheldUrls`/`withheldUrlCount`); emailrep is restricted to sender-side addresses (From/Reply-To/Return-Path/Sender) minus recipients; the legacy paste path got equivalent plain/percent/base64/base64url embed checks against To/Cc; urlscan submissions default to `unlisted` (defense in depth — analyst still gets report+screenshot, result is not publicly indexed/searchable). Also: origin IP is pinned first in the 5-IP enrichment cap (relay chains could starve the Sender Intelligence panel), and parser indicator wording no longer claims "recipient's exact identity" for tenant-domain-only matches.
2. **Email Analyzer email-first workbench** (`b632359`). Side-by-side preview/evidence split replaced by full-width tabs with the rendered email as the default Message tab (Rendered/Decoded-text toggle; Body tab folded in). New persistent **triage strip** above the tabs: verdict/SCL chip, SPF/DKIM/DMARC, origin IP chip with geo + calibrated score (auto-enriched via `/ip` the moment a parse lands — no ENRICH ALL needed), link/QR/doc-link counts, attachment risk, IDENTITY BINDING warning, expandable flags — every chip pivots. New **Links tab**: per-URL provenance (BODY / QR CODE / DOC LINK / NESTED .eml chain via the previously-invisible `nestedFrom`), wrapper unwraps, binding evidence with honest exact-vs-tenant wording, base64 decode chains, PII-withheld markers, SCAN → url-result (detonation screenshot). **Session race fixes**: all async resolvers (analyze/enrich/verdict/IP-lookup) write into their session by captured id and touch flat mirror state only if still active (ref-checked) — switching emails mid-flight no longer cross-contaminates sessions (the 2026-08-20b drawer had a verified clobber bug); busy states are per-session; paste sessions snapshot their inputs. Removed `.msg` from accept (OLE, unparseable); clipboard export failure now visible; File Manager icon off the desktop (launcher-only) since Email Analyzer holds a desktop slot.
3. **threat-intel evidence + scoring integrity** (`8264e4d`, deployed). (a) New `sourceStatus: Record<source, {ok, error?}>` persisted/returned alongside `sources` in /ip, /bulk, /deep-enrich — failed sources are no longer invisible (the Sources tab's Error pill was dead code; bulk rate-limit damage was silently under-scoring). (b) `ip_lookups.threat_score` and `saveIPScanGraph` score/verdict now come from **calibrated** scoring in all three routes — a PBL-only Spamhaus listing no longer writes `verdict='malicious'` into the graph (legacy `overallThreatScore` retained inside result JSON for compatibility). (c) /bulk runs IPs through a concurrency pool of 5 (was ~360 simultaneous fetches for a 20-IP batch).
4. **Desktop token migration + honesty** (`7095c8e`). DesktopSystemMonitor/Settings/CaseManager/GitHub local neon palettes aliased onto `palette.*` (IntelDashboard pattern); SpotlightSearch restyled from slate/cyan Tailwind onto tokens; ServiceStatus stops faking ONLINE for unconfigured Sentinel/Defender/Entra (honest "not configured", no glow); BootSequence drops fabricated CPU/RAM specs, scanlines, and "ThamOSX v7.0.0" branding (now ThamOS T6, real boot steps); assorted glow/hex cleanups (Taskbar, VPS Terminal, Terminal); Taskbar nested-`<button>` a11y fix.
5. **Result pages reflow with their window** (`090bbec`). Root cause of the maximized-window bug: Tailwind viewport breakpoints don't track window size. Added `@tailwindcss/container-queries` (TW 3.4), `@container` on result scroll roots (ResultShell + non-kit pages), converted layout grids across IP/URL/Domain/Hash/CVE/Wallet/Email results to `@xl/@3xl/@5xl` variants; removed `max-w-4xl` hard caps (Wallet/Email); IPResult Sources tab wired to `sourceStatus` for honest OK/Error pills; `TASKBAR_HEIGHT=48` unified (tiling used 44 → windows overlapped taskbar 4px).
6. **Spamhaus DoH error-code filter + Bulk CSV** (`36fe27b`, deployed). Spamhaus answers in 127.255.255.0/24 (resolver-blocked/rate-limited codes) no longer count as "listed" — previously a Spamhaus rate-limit on Cloudflare DoH would have flipped every IP to listed. Bulk CSV export gained RFC-4180 quoting (commas in ISP names shifted columns) + formula-injection guard.

**Verified:** `tsc --noEmit` clean (repo baseline is now 0 errors), `npm run build` clean (pre-existing chunk warning only), eslint no new issues. Email workbench smoke-tested with a throwaway Playwright harness (temp qa.html mounting EmailAnalyzer behind the auth wall, placeholder env, paste-mode synthetic sample): triage strip, tabs, Message/Links/Headers, flags expansion, error-path Sender Intelligence, and two-session drawer switching all verified from screenshots; harness (qa.html, src/qa-main.tsx, .env.local) deleted afterward. Edge deploys: threat-intel ×3 (final has everything), analyze-email ×2, email-verdict ×1 — all succeeded. Live in-app test with a real signed-in session still pending, as with prior sprints.

**Remaining backlog from the assessment (verified findings, deliberately not done this sprint):**
- *Email:* Print Report is still a bare `window.print()` with no print stylesheet (broken deliverable in a dark windowed app — needs a dedicated print/report view); RFC 2047 adjacent-encoded-word fixup in email-parser is dead code (long UTF-8 subjects get corrupted joins); RFC 2231 extended filenames unhandled; URL/IP/email extractors quadruplicated with drifting private-IP filters; QR time budget only checked between images and the 40 MP ceiling permits ~160 MB RGBA buffers in an edge function; email-verdict sends body + recipient to the model provider (user's own key) with no UI disclosure line; PTR/blacklist rollup + rendered thread view still backlog.
- *Desktop:* DesktopContext re-render storm (provider value recreated every render, no memoization — every mousemove during drag re-renders all windows' content); Ctrl+W/Ctrl+Tab/Ctrl+1-4 are browser-reserved (Ctrl+W closes the actual tab, guaranteed while an input is focused); restoring a minimized+maximized window silently un-maximizes (`restoreWindow` clears both flags); closing all windows never persists (empty layout skipped → closed windows resurrect on reload); windows (z 101+) render over the taskbar (z-50); ShortcutsOverlay/context menu label Ctrl+K "App Launcher" but it opens Spotlight; Ctrl+Shift+T reopens result windows without their `data`; Settings accent-color picker writes `thamos6-accent` which nothing reads; calendar popover on clock and same-app taskbar grouping still absent.
- *IP/platform:* Terminal-theme `TerminalIPResult` and Tactical `IPLookup.tsx` still render legacy scores/verdicts (two divergent render paths); /hash and /domain routes still write legacy `threat_score`; audit-event payloads still log legacy scores; plaintext `api_key` column fallback when decryption fails; likely-dead sources (ip.teoh.io, anonymous CIRCL pDNS in FREE_SOURCES, mnemonic pDNS) — now at least *visible* as errors via sourceStatus; `threat-intel/index.ts` is a 3,900-line monolith with the dispatch block copy-pasted across three routes (decompose before adding the next scanner route); shared `anon` cache context note stands.

**Decisions:**
- The PII gate withholds recipient-bound URLs from ALL external URL scanners rather than submitting privately — per the standing boundary in this file ("never transmit recipient PII to third-party services"). The analyst still gets domain-level enrichment plus full local evidence; the user's "or just send that also" musing was resolved in favor of the documented boundary.
- `sourceStatus` was added as a parallel field instead of wrapping `sources` values in `{data, error}` — every existing consumer reads `sources.<name>` as the raw provider payload, so the /email-route wrapper pattern would have broken them.
- Desktop icon set: Email Analyzer keeps its desktop slot; File Manager demoted to launcher-only per user's "maybe move File Manager".
